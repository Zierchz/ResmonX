...
- One backend module per subsystem under `monitor/`: `cpufreq` (PDH), `pdh` (memory + per-disk counters), `gpu` (NVML), `battery` (battery IOCTLs), `net` (netstat2), `services` (SCM), `threads` (Toolhelp), `etw` (ETW session), `control` (actions), `icons` (exe icon → PNG). `ipc.rs` (pipe + elevation) and `commands.rs` (`Backend` + command wrappers) sit at `src/` root.
