...
- One backend module per subsystem under `monitor/`: `cpufreq` (PDH), `pdh` (memory + per-disk counters), `gpu` (NVML), `battery` (battery IOCTLs), `net` (netstat2), `services` (SCM), `threads` (Toolhelp), `etw` (ETW session), `control` (process actions + TCP connection closing), `firewall` (WFP block rules), `icons` (exe icon → PNG). `ipc.rs` (pipe + elevation) and `commands.rs` (`Backend` + command wrappers) sit at `src/` root.
...
- **Network control also needs elevation.** `firewall.rs` blocks per-executable or per-remote-IP with user-mode WFP filters (no kernel driver, like simplewall) in its own sublayer. The engine session is `FWPM_SESSION_FLAG_DYNAMIC`, so Windows drops every filter when the helper exits — rules are intentionally non-persistent and nobody can be left offline. WFP matches on the image path, so blocking one process blocks every instance of that `.exe`; blocking also closes the live TCP connections, since a filter is only consulted when a connection is established. Rate limiting is out of scope: throttling needs a kernel callout driver. `control.rs` closes single connections with `SetTcpEntry` + `MIB_TCP_STATE_DELETE_TCB`, which is **IPv4-only** (no public IPv6 equivalent), so the UI disables that action on IPv6 rows.
